...
public class Gridder extends javax.swing.JFrame 
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        // Randomly turn a square to white
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        // Clear the grid
        clearGrid();
        // Turn the top half grid to white
        for (int i = 0; i < gridCount; i++) {
            for (int j = 0; j < gridCount/2; j++) {
                grid[i][j] = 1;
            }
        }
        // Turn the bottom half grid to black
        for (int i = 50; i < gridCount; i++) {
            for (int j = 50; j < gridCount/2; j++) {
                grid[i][j] = 0;
            }
        }
        draw();
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        int count = 0;
        // Count the number of squares that are white
        for (int i = 0; i < gridCount; i++) {
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        // Clear the grid
        clearGrid();
        // Create a white X by changing all (a, a) and (a, 99-a) squares to white
        for (int i = 0; i < gridCount; i++) {
            grid[i][i] = 1;
    ...
    private void jButton6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        // Inverse the grid by changing all black squares to white, white squares to black
        for (int i = 0; i < gridCount; i++) {
            for (int j = 0; j < gridCount; j++) {
                if (grid[i][j] == 1) {
                    grid[i][j] = 0;
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        // Create a temporary grid and a counter to track the number of white squares touched
        ...
        for (int row = 1; row < gridCount - 1; row++) {
            for (int col = 1; col < gridCount - 1; col++) {
                ...
                if(grid[col - 1][row - 1] == 1) { // check upper left
                    count++;
                }
                if(grid[col][row - 1] == 1) { // check above
                    count++;
                }
                if(grid[col + 1][row - 1] == 1) { // check upper right
                    count++;
                }
                if(grid[col + 1][row] == 1) { // check to the right
                    count++;
                }
                if(grid[col + 1][row + 1] == 1) { // check lower right
                    count++;
                }
                if(grid[col][row + 1] == 1) { // check below
                    count++;
                }
                if(grid[col - 1][row + 1] == 1) { // check lower left
                    count++;
                }
                if(grid[col - 1][row] == 1) { // check to the left
                    count++;
                }
                // If the square touches 2 white squares, turn it to white
                if (count == 2) {
                    temp[col][row] = 1;
                }
                // Initialize count
    ...
    private void jButton8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        // Move all squares one position to the left
        for (int row = 0; row < gridCount; row++) {
            for (int col = 0; col < gridCount; col++) {
                // Move the leftmost column to the rightmost column
                if (col == gridCount - 1) {
                    temp[col][row] = grid[0][row];
                }
                else {
                    temp[col][row] = grid[col + 1][row];
                }
            }
        }
        // Make grid equal temp
    ...
    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        // Rotate the grid 90 degrees to the right
        for (int row = 0; row < gridCount; row++) {
            for (int col = 0; col < gridCount; col++) {
